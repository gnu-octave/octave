...
classdef digraph
  ...
  methods
    ...
    function c = centrality (G, type, varargin)
      ...
      ## treatment of dangling nodes (nodes with zero outdegree).
      ## Stored edge weights are honoured as transition probabilities.
    ...
    function P = isomorphism (G1, G2, varargin)
      ...
      ## are ignored; only the directed adjacency structure
      ## (including edge multiplicities for a multigraph and
      ## self-loop multiplicities) is used.  The underlying search
      ...
      [nc1, nc2, ec1, ec2] = ...
        __isomorphism_parse_opts__ (G1, G2, true, varargin);

      A1 = adjacency (G1);
      A2 = adjacency (G2);
      [perm, found] = ...
        __isomorphism_vf2__ (A1, A2, true, nc1, nc2, ec1, ec2);
      if (! found)
        P = [];
        return;
      endif
...
## ------------------------------------
## `digraph` is a value classdef (no `< handle` supertype), so
## every assignment creates a full copy and every mutator method
## (`addnode`, `addedge`, `rmnode`, `rmedge`, `reordernodes`,
## `flipedge`, `subgraph`, ...) returns a new digraph rather than
## mutating its argument in place.  MATLAB's built-in digraph
## class has the same semantics.  These BIST blocks lock the copy
## contract in: assigning `G2 = G1` followed by `G2 = f(G2, ...)`
## must not disturb `G1`, no matter which mutator is called, how
## deep the copy chain is, or whether the digraph carries weights
## or node names.  A future refactor that redeclared
## `classdef digraph < handle` (reference semantics) would trip
## every test in this section.

## BIST — US-PS04: shared fixtures for the value-copy suite.
%!shared Gd_ps04, Gdw_ps04, Gdn_ps04
%! Gd_ps04  = digraph ([1 2 3], [2 3 1]);
%! Gdw_ps04 = digraph ([1 2 3], [2 3 1], [10 20 30]);
%! Gdn_ps04 = digraph ([1 2 3], [2 3 1], [10 20 30], {"a","b","c"});
...
## when the copy gains an extra weighted edge.
%!test <*PS04>
%! G1 = Gdw_ps04;
%! G2 = G1;
%! G2 = addedge (G2, 1, 1, 99);
%! assert (G1.Edges.Weight, [10; 20; 30]);
%! assert (numedges (G1), 3);
...
%! assert (G1.Edges.EndNodes, [1 2; 2 3; 3 1]);
%! assert (G1.Edges.Weight,   [10; 20; 30]);
...
## BIST — US-PS04: flipedge on a copy does not flip the source's
## edges (digraph-specific).
%!test <*PS04>
%! G1 = Gdw_ps04;
%! G2 = G1;
%! G2 = flipedge (G2, 1);
%! assert (G1.Edges.EndNodes(1,:), [1 2]);
...
## BIST — US-PS04: passing a digraph into a local function and
