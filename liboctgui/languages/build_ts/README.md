...
Before a new release of Octave, the language files `liboctgui/languages/*.ts`
...
- `liboctgui/languages/build_ts/update_ts_files`

which also determines the required search paths for each language.

Besides source folders within `liboctgui`, the search path is extended by some
Qt and Qsci files, collected in

- `liboctgui/build_ts/octave-qt` and
- `liboctgui/build_ts/octave-qsci`
...
`liboctgui/languages/*.ts`:

1. Change into `liboctgui/languages`
...
3. Change into `liboctgui/languages/build_ts`
