BUILT_PARSE_TREE_INC = \
  %reldir%/oct-gperf.h \
  %reldir%/oct-parse.h

BUILT_SOURCES += $(BUILT_PARSE_TREE_INC)
...
noinst_LTLIBRARIES += %reldir%/libparse-tree.la

%canon_reldir%_libparse_tree_la_SOURCES := $(PARSE_TREE_SRC)

dist_%canon_reldir%_libparse_tree_la_SOURCES := $(BUILT_PARSE_TREE_INC)

%canon_reldir%_libparse_tree_la_CPPFLAGS = \
  $(liboctinterp_liboctinterp_la_CPPFLAGS) \
  $(OCTAVE_PARSER_CPPFLAGS)

liboctinterp_liboctinterp_la_LIBADD += %reldir%/libparse-tree.la

## Special rules for sources which must be built before rest of compilation.

## Built files should generally appear in build directory.  However, output
## from Bison and Lex is regarded as source code and according to GNU
## recommendations should be placed in source directory.
## The following files are affected: oct-gperf.h, oct-parse.h, oct-parse.cc.

## Don't use a pipeline to process gperf output because if gperf is missing,
## but sed is not, the exit status of the pipeline will still be success, and
## we will end up creating an empty oct-gperf.h file.
