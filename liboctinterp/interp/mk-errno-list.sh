...
################################################################################
## Usage: mk-errno-list.sh mk-errno-list [--perl PERL|--python PYTHON|--sed SED]
## Purpose: Create code file "oct-errno.cc" by substituting list of system
## error names available.
###############################################################################

if [ $# -ne 2 ]; then
  echo "usage: mk-errno-list [--perl PERL|--python PYTHON|--sed SED]" 1>&2
  exit 1
fi
