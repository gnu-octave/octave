...
classdef graph
  ...
  methods
    ...
    function h = plot (G, varargin)
      ...
      ## Render the undirected graph @var{G} on the current axes and
      ## return a @code{GraphPlot} handle.
      ##
    ...
    function s = saveobj (G)
      ...
      ## Serialize the @code{graph} object @var{G} to a scalar struct
      ## suitable for @code{save}.
      ##
      ...
      ## @table @asis
      ## @item @code{adj_}
      ## The symmetric sparse adjacency matrix.
      ##
      ...
      ## @end table
      ##
      ## The struct is a pure-data representation and does @emph{not}
      ## include the Dependent @code{Edges} and @code{Nodes} views.
      ## The struct round-trips through @code{save}/@code{load} and
      ## may be handed back to @code{graph.loadobj} to reconstruct
      ## the original graph.
      ##
  ...
  methods (Static)
    ...
    function G = loadobj (s)
      ...
      ## -*- texinfo -*-
      ## @deftypefn  {} {@var{G} =} graph.loadobj (@var{s})
      ## Reconstruct a @code{graph} from a saved representation.
      ##
      ## @var{s} is either a scalar struct containing the private
      ## internal state produced by @code{saveobj} (or by Octave's
      ## default classdef-to-struct conversion when a graph is saved
      ## with @code{save}), or an existing @code{graph} object (in
      ## which case it is returned unchanged).
      ##
      ## Typical usage after loading a graph saved to a MAT-file:
      ##
      ## @example
      ## @group
      ...
      ## @end example
      ##
      ## The explicit @code{graph.loadobj} call is needed because
      ...
      if (! isstruct (s) || ! isscalar (s))
        error ("Octave:invalid-input-arg", ...
               "graph.loadobj: S must be a scalar struct or graph");
      endif
...
## internal fields needed to reconstruct the graph.
%!test <*PS01>
%! G = graph ([1 2 3], [2 3 1], [10 20 30]);
%! s = saveobj (G);
%! assert (isstruct (s));
...
## BIST — US-PS01: graph.loadobj(struct) reconstructs a graph from
## the struct returned by saveobj.
%!test <*PS01>
%! G = graph ([1 2 3], [2 3 1], [10 20 30]);
%! s = saveobj (G);
%! H = graph.loadobj (s);
...
## BIST — US-PS01: graph.loadobj(graph) is idempotent — if the input
## is already a graph (e.g., load preserved the class), it is
## returned unchanged.
%!test <*PS01>
%! G = graph ([1 2], [2 3]);
%! H = graph.loadobj (G);
...
%! warning ("off", "Octave:classdef-to-struct", "local");
%! warning ("off", "Octave:load:classdef-to-struct", "local");
%! G0 = graph ([1 2 3], [2 3 1], [1.5 2.5 3.5]);
%! mat_file = [tempname() ".mat"];
%! unwind_protect
%!   G = G0;
%!   save ("-v7", mat_file, "G");
...
%! warning ("off", "Octave:classdef-to-struct", "local");
%! warning ("off", "Octave:load:classdef-to-struct", "local");
%! G0 = graph ([1 2 3], [2 3 1], [10 20 30], {"A","B","C"});
%! mat_file = [tempname() ".mat"];
%! unwind_protect
%!   G = G0;
%!   save ("-v7", mat_file, "G");
...
## BIST — US-PS01: save -v7 / load round-trip of the empty graph.
%!test <*PS01>
%! warning ("off", "Octave:classdef-to-struct", "local");
%! warning ("off", "Octave:load:classdef-to-struct", "local");
%! G0 = graph ();
...
## graph (node count preserved; no edges).
%!test <*PS01>
%! warning ("off", "Octave:classdef-to-struct", "local");
%! warning ("off", "Octave:load:classdef-to-struct", "local");
%! G0 = graph (5);
...
## BIST — US-PS01: loadobj errors when given a non-struct, non-graph.
%!error <scalar struct or graph>
%! graph.loadobj (42);

## BIST — US-PS01: loadobj errors on a non-scalar struct.
%!error <scalar struct or graph>
%! graph.loadobj (struct ("adj_", {sparse(0,0), sparse(0,0)}));
...
## property 'Edges' of graph".  These BIST blocks lock both
## directions in so that a future refactor cannot silently break
## the read-only contract.  Undirected graph does not support a
## 'multigraph' constructor flag (scripts/graph/graph.m only
## accepts cellstr or scalar integer as the 4th argument), so
## the multigraph fixture from digraph.m has no counterpart here.

## BIST — US-PS02: shared fixtures for the subsref/subsasgn suite.
%!shared Gu_ps02, Gun_ps02
%! Gu_ps02  = graph ([1 2 3], [2 3 1], [10 20 30]);
%! Gun_ps02 = graph ([1 2 3], [2 3 1], [10 20 30], {"a","b","c"});

## --- READ path: chained subsref returns the expected data ---

## BIST — US-PS02: G.Edges.Weight returns the weight column
## vector, in the lex (s<=t) order produced by find(tril(adj_)).
## For the test fixture the constructor-supplied edges (1-2),
## (2-3), (3-1) become (1-2), (1-3), (2-3) with weights [10;30;20].
%!test <*PS02>
%! assert (Gu_ps02.Edges.Weight, [10; 30; 20]);

## BIST — US-PS02: G.Edges.EndNodes returns m-by-2 matrix sorted
## by (s, t) with s <= t.
%!test <*PS02>
%! assert (Gu_ps02.Edges.EndNodes, [1 2; 1 3; 2 3]);

## BIST — US-PS02: G.Nodes.Name returns a column cellstr for a
## named graph.
%!test <*PS02>
%! assert (Gun_ps02.Nodes.Name, {"a"; "b"; "c"});

## BIST — US-PS02: G.Nodes.Name on an unnamed graph returns empty
## cell(0,1).
%!test <*PS02>
%! assert (Gu_ps02.Nodes.Name, cell (0, 1));

## BIST — US-PS02: G.Edges.Weight(i) scalar indexed read.
%!test <*PS02>
%! assert (Gu_ps02.Edges.Weight(2), 30);

## BIST — US-PS02: G.Edges.Weight(end) end-indexing works.
%!test <*PS02>
%! assert (Gu_ps02.Edges.Weight(end), 20);

## BIST — US-PS02: G.Edges.EndNodes(i,:) row read.
%!test <*PS02>
%! assert (Gu_ps02.Edges.EndNodes(2,:), [1 3]);

## BIST — US-PS02: G.Edges.EndNodes(:,1) column read.
%!test <*PS02>
%! assert (Gu_ps02.Edges.EndNodes(:,1), [1; 1; 2]);

## BIST — US-PS02: G.Nodes.Name{i} brace-indexed read.
%!test <*PS02>
%! assert (Gun_ps02.Nodes.Name{2}, "b");

## BIST — US-PS02: numel (G.Edges.Weight) equals numedges (G).
%!test <*PS02>
%! assert (numel (Gu_ps02.Edges.Weight), numedges (Gu_ps02));

## BIST — US-PS02: dynamic-field access G.("Edges") / G.("Nodes")
## equals the static form.
%!test <*PS02>
%! assert (isequal (Gu_ps02.("Edges"), Gu_ps02.Edges));
%! assert (isequal (Gun_ps02.("Nodes"), Gun_ps02.Nodes));
